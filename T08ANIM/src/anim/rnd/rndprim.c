...
VOID IK1_RndPrimDraw( ik1PRIM *Pr, MATR World )
{
  ...
  for (i = 0; i < Pr->NumOfV; i++)
  {
    ...
    pnts[i].x = (INT)((p.X + 1) * IK1_RndFrameW / 2);
    pnts[i].y = (INT)((-p.Y + 1) * IK1_RndFrameH / 2);
  }
  SelectObject(IK1_hRndDCFrame, GetStockObject(DC_PEN));
  SetDCPenColor(IK1_hRndDCFrame, RGB(130, 0, 130));
  for (i = 0; i < Pr->NumOfI; i += 3)
  {
    MoveToEx(IK1_hRndDCFrame, pnts[Pr->I[i]].x, pnts[Pr->I[i]].y, NULL); 
    LineTo(IK1_hRndDCFrame, pnts[Pr->I[i + 1]].x, pnts[Pr->I[i + 1]].y);
    LineTo(IK1_hRndDCFrame, pnts[Pr->I[i + 2]].x, pnts[Pr->I[i + 2]].y);
    LineTo(IK1_hRndDCFrame, pnts[Pr->I[i]].x, pnts[Pr->I[i]].y);
  }
  free(pnts);
}
...
BOOL IK1_RndPrimLoad( ik1PRIM *Pr, CHAR *FileName )
{
  ...
  if (!IK1_RndPrimCreate(Pr, nv, nf * 3))
  {
    fclose(F);
    return FALSE;
  }
...
BOOL IK1_RndPrimCreateSphere( ik1PRIM *Pr, DBL R, INT W, INT H )
{
  ...
  if (!IK1_RndPrimCreate(Pr, W * H, (H - 1) * (W - 1) * 2 * 3))
    return FALSE;
